defmodule Cybernetic.Core.Transport.AMQP.Topology do
  ...
  @queues [
    ...
    {"cyb.events.retry",
     ...
     arguments: [
       ...
       {"x-message-ttl", :signedint, 15_000}
     ]},
  ...
  def declare_exchanges(channel) do
    ...
    Enum.reduce_while(exchanges, :ok, fn {key, exchange_name}, _acc ->
      type = Map.get(exchange_types, key, :topic)
      safe_declare_exchange(channel, exchange_name, type, key)
    end)
    |> handle_exchange_result(channel)
  end

  defp safe_declare_exchange(channel, name, type, key) do
    case Exchange.declare(channel, name, type, durable: true, auto_delete: false) do
      :ok ->
        Logger.debug("Declared exchange: #{key}=#{name} (#{type})")
        {:cont, :ok}

      {:error, {:resource_locked, _}} ->
        Logger.debug("Exchange already exists: #{name}")
        {:cont, :ok}

      {:error, reason} ->
        Logger.error("Failed to declare exchange #{key}=#{name}: #{inspect(reason)}")
        {:halt, {:error, reason}}
    end
  end

  defp handle_exchange_result(:ok, channel) do
    # Hardcoded special exchanges not in config
    special_exchanges = [
      {"vsm.dlx", :fanout},
      {"cybernetic.exchange", :topic}
    ]

    Enum.reduce_while(special_exchanges, :ok, fn {name, type}, _acc ->
      safe_declare_exchange(channel, name, type, name)
    end)
  end

  defp handle_exchange_result(error, _channel), do: error
  ...
  def declare_queues(channel) do
    Enum.reduce_while(@queues, :ok, fn queue_spec, _acc ->
      ...
      safe_declare_queue(channel, name, opts)
    end)
  end

  defp safe_declare_queue(channel, name, opts) do
