...
When you are finished, summarize your work and stop.`,
    ...
    while (loopCount < MAX_LOOPS && !completionDetected) {
      ...
      const xmlCalls = result.content
        ? parseToolCalls(result.content, logger)
        : [];
      ...
      if (allCalls.length > 0) {
        ...
        for (const call of allCalls) {
          ...
          const toolParams =
            (call as any).args || (call as any).function?.params;
          ...
          try {
            ...
            if (toolDef) {
              const args =
                typeof toolParams === "string"
                  ? JSON.parse(toolParams)
                  : toolParams;
              toolResult = (await toolDef.func(args)) as string;
            } else {
