...
import type { DreamMcpHandlers } from "../../agent/mcp/dreamMcpServer";
...
// Capture the onDreamIdeas handler when createDreamMcpServer is called
let capturedDreamHandler: DreamMcpHandlers["onDreamIdeas"] | undefined;

mock.module("../../agent/mcp/dreamMcpServer", () => ({
  createDreamMcpServer: mock((handlers: DreamMcpHandlers) => {
    capturedDreamHandler = handlers.onDreamIdeas;
    return { name: "wreckit-dream", tools: [] };
  }),
}));

// Mock runAgentUnion to simulate agent calling the dream handler
mock.module("../../agent/runner", () => ({
  runAgentUnion: mock(async () => {
    const ideas = [
      {
        title: "[DREAMER] Fix TODO in auth.ts",
        description: "Add rate limiting as noted in TODO comment",
    ...
    // Trigger the captured handler (simulating MCP tool call)
    capturedDreamHandler?.(ideas);
...
describe("Dream Command Integration", () => {
  ...
  it("should run the agent and save ideas", async () => {
    ...
    expect(mockLogger.info).toHaveBeenCalledWith(
      "Autonomous ideation complete.",
    );
    expect(mockLogger.info).toHaveBeenCalledWith(
      expect.stringContaining("Generated: 1 ideas"),
    );
  });
  ...
  it("should enforce [DREAMER] prefix", async () => {
    ...
    expect(mockLogger.info).toHaveBeenCalledWith(
      expect.stringContaining("New items:"),
    );
  });
});
